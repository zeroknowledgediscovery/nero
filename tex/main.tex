...
\keywords{entropy rate $|$ %% AI-generated text detection   $|$
large language models  $|$  algorithmic complexity  $|$ probabilistic  automata }
...
\begin{abstract}
We introduce a model-agnostic, training-free estimator of intrinsic complexity, quantified as entropy rate, for long-form text streams under a fixed coarse-grained alphabet. Across multiple contemporary large language models (LLMs) and human-authored corpora, LLM outputs occupy systematically lower entropy-rate regimes than human prose under this shared representation. The statistic is computed directly from text without access to model internals, supervision, or retraining; as a threshold score it yields strong separation of human and LLM text across model families and corpora. Interpreted through algorithmic statistics, these entropy-rate differences are consistent with differences in effective descriptive complexity of the underlying generative mechanisms. Beyond discrimination, this provides a calibration-free means to rank generative capacity and track distributional drift across successive models.%We introduce a model-agnostic, training-free estimator of intrinsic complexity, estimated as entropy rate, for longform text streams under a fixed coarse graining  alphabet. Across multiple contemporary large language models (LLMs) and human-authored corpora, we find that LLM outputs occupy systematically lower entropy-rate regimes than human prose under this shared representation. We compute this statistic directly from text without access to model internals, supervision, or retraining; used as a threshold score it yields strong separation of human and LLM text across model families and corpora. Interpreted through algorithmic statistics, the observed entropy-rate differences are consistent with differences in effective descriptive complexity of the underlying generative mechanisms. Beyond discrimination, this enables a calibration-free  ranking of  generative capacity and  tracking distributional drift in successive generative models.
%
...
\dropcap{G}enerative artificial intelligence (AI), particularly LLMs, now produce prose that increasingly overlaps human writing, making provenance nontrivial~\cite{naturebe2023prepare,crothers2023machine,thirunavukarasu2023large}. Here we hypothesize that long-form human and machine text occupy distinct regimes of statistical complexity when mapped to symbol streams over a 27-character alphabet (26 letters plus space). Treated as sample paths of a stochastic process~\cite{CoverKing1978}, intrinsic complexity, proxied by entropy rate, reveals this separation. In contrast, existing approaches~\cite{detectgpt_icml2023,detectllm_findings2023} typically evaluate generators through model-dependent scoring (e.g., likelihood or rank statistics), supervised stylistic representations, or compressibility proxies, and thus rely on reference models, training, or calibration that may drift as generators evolve.

Our Nonparametric, learning-free  Entropy-Rate Oracle (\NERO) operates directly on text, without model access, supervision, or retraining, and reveals systematically lower entropy rates in contemporary LLM outputs than in human prose under a shared symbolization, yielding consistent separation across model families and corpora. Optionally, the estimator’s substring-frequency cutoff profile can be used as features for a Gaussian process classifier, achieving AUC $=98.9\%$ substantially exceeding model-driven baselines.
Thus, \NERO-estimated entropy rate functions as a model-agnostic, process-level statistic, akin to a physical signature or order-parameter for effective generative capacity, enabling calibration-free ranking and longitudinal tracking of generative behavior across releases; discrimination follows as a corollary of our conceptual framing. Because we target a  distributional property of long-form text, task-based LLM benchmarks are not directly commensurate here (see Supplementary Information).

%% \dropcap{G}enerative artificial intelligence (AI), particularly LLMs, are now producing  prose that  increasingly overlaps human writing, making provenance nontrivial~\cite{naturebe2023prepare,crothers2023machine,thirunavukarasu2023large}. Here we  hypothesize that long-form human and machine text occupy distinct regimes of statistical complexity when mapped to symbol streams over a 27-character alphabet (26 letters plus space). When treated as sample paths of a stochastic process~\cite{CoverKing1978}, intrinsic complexity measured via the proxy of  entropy rate reveals this difference. In contrast,  existing approaches~\cite{detectgpt_icml2023,detectllm_findings2023} typically evaluate generators through model-dependent scoring (e.g., likelihood/rank statistics), supervised stylistic representations, or compressibility proxies, and thus rely on reference models, training, or calibration that can drift with evolving generators.

%% Our nonparametric, learning-free Nonparametric Entropy-Rate Oracle (\NERO)  operates directly on text, without model access, supervision, or retraining, and reveals systematically lower entropy rates in contemporary LLM outputs than in human prose under a shared symbolization, and yields strong separation across model families and corpora. Optionally, the estimator’s substring-frequency cutoff profile can be used as features for a Gaussian process readout, yielding AUC $=98.9\%$ on our pooled evaluation, exceeding model-driven baselines.
%% %
%% Thus, \NERO-estimated entropy rate functions as a model-agnostic, process-level statistic, akin to a physical quantity, for effective generative capacity, enabling calibration-free ranking and longitudinal tracking of generative behavior across releases; discrimination is a direct corollary. Because we target a distributional property of long-form text, task-based LLM benchmarks are not directly commensurate here (see Supplementary Information).
...
Thus, despite the limitations of  requiring longer texts and greater computational effort, \NERO yields a model-agnostic statistic that enables principled, learning-free tracking of generative behavior over time.
...
\paragraph{Code availability.} Implementation available under a non-commercial license at \url{https://github.com/zeroknowledgediscovery/nero}, and at zenodo (\url{https://doi.org/10.5281/zenodo.18487995}).
}
